        ...
        <div className="space-y-4">
            ...
            <div className="bg-white/70 backdrop-blur-xl border border-white/90 rounded-2xl p-4 shadow-sm flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                    <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600"><History className="w-4 h-4" /></div>
                    <div>
                        <h3 className="text-sm font-bold text-slate-900">Emergency Call Logs &amp; Audit Trail</h3>
                        <p className="text-[11px] text-slate-400">Searchable historical incident logs with 24kHz audio recording playback</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="flex bg-slate-100 p-0.5 rounded-xl text-[10px] font-mono font-bold">
                        {['ALL', 'AI Resolved', 'Handed Over', 'Dispatched'].map(s => (
                            <button
                                ...
                                className={`px-2 py-1 rounded-lg transition-all ${statusFilter === s ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500'}`}
                            >
                                {s}
                            </button>
                        ))}
                    ...
                    <button
                        ...
                        className="py-1.5 px-3 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs border border-indigo-200 flex items-center gap-1 transition-colors"
                    >
                                ...
                                <tr key={row.callId} className="hover:bg-slate-50/70 transition-colors">
                                    ...
                                    <td className="py-3.5 px-4">
                                        <span className={`px-2 py-0.5 rounded font-bold text-[10px] font-mono border ${
                                            ...
                                            row.risk === 'MEDIUM' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
                                        }`}>
                                            {row.risk}
                                        </span>
                                    </td>
                                    <td className="py-3.5 px-4">
                                        <span className={`px-2.5 py-0.5 rounded-md text-[11px] font-bold border ${row.statusColor}`}>
                                            {row.status}
                                        </span>
                                    </td>
                                    <td className="py-3.5 px-4 text-right">
                                        <button
                                            ...
                                            className={`p-2 rounded-xl border transition-all inline-flex items-center justify-center ${
                                                playingId === row.callId ? 'bg-indigo-600 text-white border-indigo-700 shadow-sm animate-pulse' : 'bg-white hover:bg-slate-100 text-slate-600 border-slate-200'
                                            }`}
