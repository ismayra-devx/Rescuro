...
class EchoSphereOrchestrator:
    ...
    def __init__(
        ...
        self._event_subscribers: List[Callable[[EchoSphereEvent], Any]] = []

    def subscribe(self, callback: Callable[[EchoSphereEvent], Any]) -> None:
    ...
    async def process_transcript(
        ...
        asyncio.create_task(
            self.supabase_service.persist_ticket(
                session_id=session_id,
                ticket_id=ticket_id,
                summary=llm_result.incident_type or transcript,
                caller_name=(llm_result.extracted_slots or {}).get("caller_name"),
                location=llm_result.location,
                issue=llm_result.incident_type,
                confidence=triage_result.combined_confidence,
                status="escalated" if triage_result.route != "automated" else "open",
            )
        )
        ...
        if triage_result.route == "automated":
            ...
            # Asynchronously dispatch Slack alert for emergency
            context_packet = {
                "session_id": session_id,
                "call_sid": session.call_sid,
                "caller_phone": session.from_number or "Anonymous",
                "ticket_id": ticket_id,
                "caller_name": (llm_result.extracted_slots or {}).get("caller_name"),
                "location": llm_result.location,
                "issue": llm_result.incident_type or transcript,
                "language": "Hinglish",
                "confirmed_slots": llm_result.extracted_slots or {},
                "missing_slots": [],
                "combined_confidence": triage_result.combined_confidence,
                "confidence_threshold": settings.CONFIDENCE_ESCALATION_THRESHOLD,
                "escalation_reason": triage_result.reason,
                "safety_flag": True,
                "channel_name": f"echosphere_{session_id[:8]}",
            }
            asyncio.create_task(slack_service.send_escalation_alert(context_packet))
        ...
        return {
            ...
            "status": session.status.value,
            "tts_halted": session.tts_halted,
            "conversational_reply": llm_result.reply,
        }
    ...
    async def supervisor_override(
        ...
        # Dispatch Slack alert for supervisor takeover
        esc_ticket_id = f"TKT-ESC-{uuid.uuid4().hex[:6].upper()}"
        context_packet = {
            "session_id": session_id,
            "call_sid": session.call_sid,
            "caller_phone": session.from_number or "Anonymous",
            "ticket_id": esc_ticket_id,
            "caller_name": "Caller",
            "location": "See live dashboard",
            "issue": session.latest_transcript or "Supervisor Takeover",
            "language": "Hinglish",
            "confirmed_slots": {},
            "missing_slots": [],
