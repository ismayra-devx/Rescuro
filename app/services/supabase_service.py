...
from app.models.events import EchoSphereEvent
...
class SupabaseService:
    ...
    def __init__(
        ...
        self._tickets_table: List[Dict[str, Any]] = []
    ...
    # =========================================================================
    async def persist_event(self, event: EchoSphereEvent) -> Dict[str, Any]:
    ...
    async def persist_ticket(
        ...
        """Insert or update a trouble ticket associated with a call session."""
        record = {
            "id": ticket_id,
            "session_id": session_id,
            "summary": summary,
        ...
        existing = next((t for t in self._tickets_table if t["id"] == ticket_id), None)
        if existing:
            existing.update(record)
        else:
            self._tickets_table.append(record)

        if self._is_live_configured:
            try:
                async with httpx.AsyncClient(timeout=5.0) as client:
                    await client.post(
                        f"{self.supabase_url}/rest/v1/tickets",
                        headers=self._get_headers(),
                        json=record,
                    )
            except Exception as exc:
                logger.warning(f"Live Supabase persist_ticket failed ({exc}), using memory.")

        logger.info(f"Persisted ticket {ticket_id} for session {session_id}")
        return record

    async def get_tickets(self, session_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Retrieve tickets, optionally filtered by session_id."""
        if self._is_live_configured:
            try:
                endpoint = f"{self.supabase_url}/rest/v1/tickets?order=created_at.desc"
                if session_id:
                    endpoint += f"&session_id=eq.{session_id}"
                async with httpx.AsyncClient(timeout=5.0) as client:
                    res = await client.get(endpoint, headers=self._get_headers())
                    if res.status_code == 200:
                        return res.json()
            except Exception as exc:
                logger.warning(f"Live Supabase get_tickets failed ({exc}), using memory.")

        if session_id:
            return [t for t in self._tickets_table if t["session_id"] == session_id]
        return list(self._tickets_table)
    ...
    async def delete_session(self, session_id: str) -> None:
        ...
        self._tickets_table = [tk for tk in self._tickets_table if tk["session_id"] != session_id]
